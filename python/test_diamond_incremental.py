...
def test_letter_sequence_is_list_of_letters_on_each_line_of_the_diamond():
...
def test_indents_is_list_of_indentation_for_each_line_of_the_diamond():
...
def test_between_is_list_of_how_many_middle_spaces_between_the_repeated_letter_for_each_line_of_the_diamond():
...
def test_one_row_is_a_list_representing_one_diamond_row():
...
def test_rows_is_a_list_of_all_diamond_rows():
	assert diamond.Diamond('A').rows() == ["A"]
	assert diamond.Diamond('B').rows() == [" A", "B B", " A"]

def test_DiamondC_prints_correctly():
